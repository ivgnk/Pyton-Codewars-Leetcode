...
A plate is considered between candles if there is at least one candle
...
# Memory Limit Exceeded -- 12 / 36 testcases passed
...
def platesBetweenCandles(s, queries):
    ...
    :rtype: List[int]
    """
    q1 = [s[q[0]:q[1]+1] for q in queries]
    q2=[q.lstrip('*').rstrip('*') for q in q1]
    return [q.count('*') for q in q2]

print(platesBetweenCandles("**|**|***|",queries = [[2,5],[5,9]]))