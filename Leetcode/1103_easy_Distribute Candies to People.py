...
# Runtime 19 ms Beats 60.87% of users with Python
# Memory 11.77 MB Beats 34.78% of users with Python
...
def distributeCandies(candies, num_people):
    ...
    while (ncandy>=0):
        for i, dat in enumerate(lst):
            # print(i,'b', ncandy, sumcandy, lst)
            ncandy = ncandy +1
            sumcandy = sumcandy+ncandy
            lst[i] = lst[i] + ncandy
            if ncandy<0: break
            if sumcandy+(ncandy+1)>candies:
                ncandy=candies-sumcandy-1 #; print(f'{ncandy=}')
        #     print(i,'a', ncandy, sumcandy,lst,'\n')
        # print('wh',candies, lst)
        # print(); input()
    return lst
