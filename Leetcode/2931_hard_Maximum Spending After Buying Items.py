...
items in m different shops. Each shop has n items where the jth item in the ith shop has a value of values[i][j].
...
# Runtime 667 ms Beats 100.00% of users with Python
# Memory 26.00 MB Beats 100.00% of users with Python
...
def maxSpending(values):
    ...
    ll=len(a)
    a=sorted(a)
    return sum([(i+1)*a[i] for i in range(ll)])
