...
Hint 2
Consider each letter one at a time.
If there are x occurrences of a letter in s and y occurrences of the same letter in target,
how many copies of this letter can we make?
Hint 3
We can make floor(x / y) copies of the letter.

Solution
✏️ Easy Python solution with dictionary
Intuition
Make dictionary for s and target
...
Find min (floor(x / y) copies of the letter)
"""

# Runtime 12 ms Beats 82.46%
# Memory 11.66 MB Beats 54.39%
...
def rearrangeCharacters(s, target):
    ...
    """
    d_s =dict(Counter(s))
    d_t =dict(Counter(target))
    res=[]
    for k,v in d_t.items():
        if k not in d_s: return 0
        else: res.append(int(d_s[k]/v))
    return min(res)
