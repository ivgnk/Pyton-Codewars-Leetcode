"""
Done 09.07.2024. Topics: Array, Simulation
...
Output: 5.00000
Explanation:
1) The first customer arrives at time 1,
the chef takes his order and starts preparing it immediately at time 1,
...
3) The third customer arrives at time 4,
the chef takes his order and starts preparing it at time 8,
...
Output: 3.25000
Explanation:
1) The first customer arrives at time 5,
...
2) The second customer arrives at time 5,
the chef takes his order and starts preparing it at time 7, and finishes at time 11,
so the waiting time of the second customer is 11 - 5 = 6.
3) The third customer arrives at time 10,
the chef takes his order and starts preparing it at time 11, and finishes at time 14,
so the waiting time of the third customer is 14 - 10 = 4.
4) The fourth customer arrives at time 20,
the chef takes his order and starts preparing it immediately at time 20,
...
# Runtime 747 ms Beats 61.51%
# Time Complexity O(N)
# Memory 57.29 MB Beats 9.97%
...
def averageWaitingTime(customers):
    ...
    """
    ll=len(customers)
    stime, ctime=0, 0 # sum wait time, # ctime after order end
    for i in range(ll):
        if i==0 or customers[i][0]>=ctime:
            ctime=customers[i][0]+customers[i][1]
            stime=stime+customers[i][1]
        else:
            # ord_wait=ctime-customers[i][0]
            # wt=ord_wait+customers[i][1]
            # ctime = ctime+customers[i][1]
            # stime = stime+wt
            stime = stime + ctime-customers[i][0] +customers[i][1]
            ctime = ctime+customers[i][1]

    return stime/ll

ic(averageWaitingTime([[1,2],[2,5],[4,3]]))
ic(averageWaitingTime( [[5,2],[5,4],[10,3],[20,1]]))
