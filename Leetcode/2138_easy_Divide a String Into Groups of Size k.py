...
# Runtime 3 ms Beats 100.00% of users with Python
# Memory 11.66 MB Beats 58.00% of users with Python
...
def divideString(s, k, fill):
    ...
    :rtype: List[str]
    """
    # https://ru.stackoverflow.com/questions/717303/Разбить-строку-на-n-символов-подстрок-одинаковой-длины-и-занести-в-массив

    s1=[s[x:x+k] for x in range (0, len(s), k)]
    ll=len(s1[-1])
    if ll<k:
        s1[-1]=s1[-1]+fill*(k-ll)
    return s1

print(divideString('123',2,'x'))
# print(divideString('123',2,'x'))