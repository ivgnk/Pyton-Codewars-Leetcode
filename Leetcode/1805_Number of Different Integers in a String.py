...
# Runtime 14 ms Beats 81.44% of users with Python
# Memory 11.76 MB Beats 48.45% of users with Python
...
def numDifferentIntegers(word):
    ...
    :rtype: int
    """
    x = ''.join([s1 if s1.isdigit() else ' ' for s1 in word])
    s= x.split()
    y=[int(s1) for s1 in s]
    return len(set(y))
