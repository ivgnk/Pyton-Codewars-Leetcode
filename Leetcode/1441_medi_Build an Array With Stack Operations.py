"""
Done 22.05.2024. Topics: Array, Stack, Simulation
...
# Runtime 13 ms Beats 76.54% of users with Python
# Memory 11.63 MB Beats 40.74% of users with Python
...
def buildArray(target, n):
    ...
    r=range(1,n+1)
    rl=list(r); ll=len(rl)
    if target == rl[:len(target)]: return ["Push" for i in target]
    res = []
    for i in rl:
        if i > target[-1]: break
        res.append("Push")
        if i not in target: res.append("Pop")
    return res