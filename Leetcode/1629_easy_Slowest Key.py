...
The tester wants to know the key of the keypress that had the longest duration. T
he ith keypress had a duration of releaseTimes[i] - releaseTimes[i - 1],
...
# Runtime 36 ms Beats 58.33% of users with Python
# Memory 11.86 MB Beats 50.00% of users with Python
...
def slowestKey(releaseTimes, keysPressed):
    ...
    ll = len(releaseTimes)
    s = str(keysPressed)
    time = [releaseTimes[0]] + [releaseTimes[i] - releaseTimes[i - 1] for i in range(1, ll)]
    # print(time)
    a_d = list(zip(s, time))
    # print(a_d)
    b_d = sorted(a_d)
